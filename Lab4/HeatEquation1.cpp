...
int main(int argc, char** argv)
{
	...
	if (rank == 0) {
		...
		// std::cout << "Rank " << rank << " starts at " << localStart[rank] << " and stop at " << localStop[rank] << std::endl;
		...
		for (int irank = 1; irank < nproc; irank++)
		{
			...
			// std::cout << "Rank " << irank << " starts at " << localStart[irank] << " and stop at " << localStop[irank] << std::endl;
		}		
	...
	// MPI_Barrier(MPI_COMM_WORLD);
	// usleep(10000*rank);
	// std::cout << "Rank " << rank << " received " << localRows.size() << " rows and " << localRows.size()*localRows[localRows.size()-1].size() << " elements" << std::endl;
    ...
    for (unsigned int it = 0; it < Nt; it++)
    {
       ...
       if (it%1000 == 0)
       {
			...
			{				
				...
				std::vector <double> allUnrolledVals(2*Nx*Ny);
				int valCount = 0;
