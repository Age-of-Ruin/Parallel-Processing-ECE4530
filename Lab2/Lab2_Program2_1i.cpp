...
int main(int argc, char** argv)  { 
	...
	if(rank == 0){
		...
		for (int irank = 0; irank < nproc; irank++)
		{
			...
			if (imageProcessID == IMAGE_PROCESS_TYPE_BLUR)
			{
				...
				imgDs[irank].haloRowCount = imgDs[irank].haloRowStop - imgDs[irank].haloRowStart;
	...
	if (rank != 0)
	{
		...
		MPI_Recv(&imgD, recv_count, MPI_BYTE, 0, PIXEL_DESCRIPTOR_TAG, MPI_COMM_WORLD, &status); 
	}
	...
	else 
	{
		...
		switch(imgD.imgProcessID){
			...
			case IMAGE_PROCESS_TYPE_ENHANCE:
				imageContrastEnhance(procImageIn, procImageOut, 0, 255);
				os << "./enhance" << rank << ".jpg";
				fileName = os.str();
				cv::imwrite(fileName,procImageOut);
				break;
