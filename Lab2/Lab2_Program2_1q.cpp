	...
	{ 
		...
		{ 
			...
			for (int ichannel = 0; ichannel < 3; ichannel++)
			{
				...
				if (in.at<cv::Vec3b>(irow,icol).val[ichannel] <= minval)
				{
					out.at<cv::Vec3b>(irow,icol).val[ichannel] = 0;
				}
				else if ((in.at<cv::Vec3b>(irow,icol).val[ichannel] >= maxval))
				{
					...
					out.at<cv::Vec3b>(irow,icol).val[ichannel] = (int)(slope*in.at<cv::Vec3b>(irow,icol).val[ichannel] - slope*minval);
				}
			}
		} 
	} 
...
void imageGreyScale(const cv::Mat& in, cv::Mat& out){
	...
	{ 
		...
		{ 
			...
			for (int ichannel = 0; ichannel < 3; ichannel++)
			{
				pixelAvg += (long)in.at<cv::Vec3b>(irow,icol).val[ichannel];
			}
...
void imageBlur(const cv::Mat& in, cv::Mat& out, int level, int rowstart, int rowstop){
	...
	{ 
		...
		{ 
			...
			if (irow + (2 * level + 1) < out.rows)
			{
				blurRowStop = irow + (2 * level + 1);
			}
			else
			{
				blurRowStop = out.rows;
			}
			...
			for (int ichannel = 0; ichannel < 3; ichannel++){
				...
				for (int blurRowIndex = blurRowStart; blurRowIndex < blurRowStop; blurRowIndex++)
				{
					...
					{
						channelAvg += (long)in.at<cv::Vec3b>(blurRowIndex,blurColIndex).val[ichannel];
						count++;				
					}
				...
				out.at<cv::Vec3b>(rowCount,colCount).val[ichannel] = (uchar) channelAvg;
			}
			
			colCount++;
			if(colCount == out.cols)
				colCount = 0;
		}
...
int main(int argc, char** argv)  { 
	...
	// Clock Start
	double tStart, tStop, tElapsed, tAvg;
	...
	if(rank == 0){
		...
		// Send required pixels for BLUR
		if (imageProcessID == IMAGE_PROCESS_TYPE_BLUR)
		{				
			// Store pixel data for rank 0
			procPixels.resize(imgDs[0].haloRowCount * imgDs[0].colCount);
			for (long i = 0; i < imgDs[0].haloRowCount * imgDs[0].colCount; i++)
			{
			...
			for (int irank = 1; irank < nproc; irank++)
			{
				MPI_Send(&allPixels[imgDs[irank].haloRowStart*imgDs[irank].colCount], imgDs[irank].haloRowCount*imgDs[irank].colCount, cv_contig_Vec3b, irank, PIXEL_DATA_TAG, MPI_COMM_WORLD);
			}
		...
		{			
			...
			for (int irank = 1; irank < nproc; irank++)
			{
				MPI_Send(&allPixels[imgDs[irank].rowStart*imgDs[irank].colCount], imgDs[irank].rowCount*imgDs[irank].colCount, cv_contig_Vec3b, irank, PIXEL_DATA_TAG, MPI_COMM_WORLD);
			}
		}
	...
	int numRows = 0;
	if (imgD.imgProcessID == IMAGE_PROCESS_TYPE_BLUR)
		numRows = imgD.haloRowCount;
	else
		numRows = imgD.rowCount;
	
	cv::Mat procImageIn(numRows, imgD.colCount, CV_8UC3);
		...
		switch(imgD.imgProcessID){
			...
			case IMAGE_PROCESS_TYPE_BLUR:	
				...
				if(nproc > 1)
				{
					// Processing
					if(rank == 0)
						imageBlur(procImageIn, procImageOut, imgD.blurLvl, 0, numRows - (imgD.blurLvl*2 + 1) );
					else if (rank == nproc-1)
						imageBlur(procImageIn, procImageOut, imgD.blurLvl, (imgD.blurLvl*2 + 1), numRows-1);
					else
						imageBlur(procImageIn, procImageOut, imgD.blurLvl, (imgD.blurLvl*2 + 1), numRows - (imgD.blurLvl*2 + 1));
				}
				else
				{
					imageBlur(procImageIn, procImageOut, imgD.blurLvl, 0, numRows-1);
				}
	...
	{		
		...
		for (int irank = 1; irank < nproc; irank ++)
		{			
			MPI_Recv(&allPixels[imgDs[irank].rowStart*imgDs[irank].colCount], imgDs[irank].rowCount*imgDs[irank].colCount, cv_contig_Vec3b, irank, PIXEL_DATA_TAG, MPI_COMM_WORLD, &status);	
		}
		
		// Rebuild Image from partial images from each processor
		long pixelCount = 0;
	...
	cout << "Rank " << rank	<< " took this long to finish " << tElapsed << endl;
