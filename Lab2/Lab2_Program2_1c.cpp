...
int main(int argc, char** argv)  { 
	...
	if(rank == 0){
		...
		// Send pixels to other processors
/* 		for (int irank = 1; irank < nproc; irank++)
        {
            MPI_Send(&allPixels[0], procPixelCounts[irank], cv_contig_Vec3b, irank, PIXEL_DATA_TAG, MPI_COMM_WORLD);
        } */
	...
	// Receive pixel data
/* 	vector<cv::Vec3b> procPixels(imgD.rowCount * image.cols);
	if (rank != 0)
	{
		int recv_count;
        MPI_Status status;
		...
		MPI_Recv(&procPixels, recv_count, MPI_BYTE, 0, PIXEL_DESCRIPTOR_TAG, MPI_COMM_WORLD, &status); //Receive
	} */
