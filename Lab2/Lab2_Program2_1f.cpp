...
int main(int argc, char** argv)  { 
	...
	// created cv::Mat (matrix) used for processing
	cv::Mat procImage (imgD.rowCount, imgD.colCount, CV_8UC3);
	...
	{ 
		...
		{ 
			procImage.at<cv::Vec3b>(irow,icol) = procPixels[pixelCount];
			pixelCount++;
		} 
	} 
	
	//cout << "Rank " << rank << " will now process " << procImage.rows << " rows" << endl;
