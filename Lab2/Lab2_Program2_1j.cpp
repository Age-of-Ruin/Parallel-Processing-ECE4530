...
/* 			// Determine number of pixels per processor
		...
		{			
			...
			for (int irank = 1; irank < nproc; irank++)
			{
				MPI_Send(&allPixels[procPixelIndex], imgDs[irank].rowCount * image.cols, cv_contig_Vec3b, irank, PIXEL_DATA_TAG, MPI_COMM_WORLD);
	...
	else 
	{
		...
		long pixelCount = 0;
		for (unsigned int irow = 0; irow < procImageOut.rows; irow++) 
		{ 
			for (unsigned int icol = 0; icol < procImageOut.cols; icol++) 
			{ 
...
/* 		// Send pixel data to rank 0
		if (rank != 0) 
		{
			...
			MPI_Send(&procPixels[0], imgD.rowCount * imgD.colCount, cv_contig_Vec3b, irank, PIXEL_DATA_TAG, MPI_COMM_WORLD);			
		} */
	}
...
/* 		// Store pixel data for rank 0
		for (long i = 0; i < imgDs[0].rowCount * imD.colCount; i++)
		{
			procPixels[i] = allPixels[i];
		}
		...
		for (int irank = 1; irank < nproc; irank ++)
		{
			...
			allPixels.resize(recv_count);
			
			MPI_Recv(&allPixels[0], recv_count, cv_contig_Vec3b, 0, PIXEL_DATA_TAG, MPI_COMM_WORLD, &status);
