...
//		cv::imwrite(fName, image_to_view1);
		...
		vector <unsigned char> allUnrolledVals(ny*nx);
		...
		for (int iter = 0; iter < maxiter; iter++)
		{
			...
			{				
				...
				// Rename image
				ostringstream fNameStream;
				fNameStream << "Population_iter_" << iter << ".jpg";
				string fName = fNameStream.str();
		...
		//------------		
			...
			for (int iy = 1; iy < localRows.size()-1; iy++)
			{
				...
				{	
					...
					{
						if (occupied_neighbours <= 1 || occupied_neighbours >= 4) localRows[iy][ix] = 255; // dies (over-crowding/starvation)
						if (occupied_neighbours == 2 || occupied_neighbours == 3) localRows[iy][ix] = 0; // stays alive (perfect amount of neighbors)
					}
					else if (localRows[iy][ix] == 255) // update dead elements
					{
						if (occupied_neighbours == 3)
						{
							localRows[iy][ix] = 0; // reproduction
						}
					}
				}
			} 
